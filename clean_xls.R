...
to_replace <- grepl("0", dataFlood$`Glide Num`, perl=TRUE)
dataFlood$`Glide Num`[to_replace] <- "NA"
...
to_replace <- grepl("0", dataFlood$Country, perl=TRUE)
dataFlood$Country[to_replace] <- "NA"


# Clean up Main Cause Column
to_replace <- grepl("0", dataFlood$`Main cause`, perl=TRUE)
dataFlood$`Main cause`[to_replace] <- "NA"
...
to_replace <- grepl("Tropical Cycl", df1$`Main cause_2`, perl=TRUE)
